...
// src/global.ts
var GLOBAL_INTEGRATIONS = /* @__PURE__ */ new Set();
var CURRENT_INTEGRATION;
...
function auto(obj, objDescriptors) {
...
function watch(scope, handler, options) {
  const pl = getIntegration();
  return pl?.watch?.(scope, handler, options);
}

// src/reactivity/global.ts
var KEYS_SYMBOL = Symbol("keys");
var OBSERVER_SYMBOL = Symbol("computed");
var TARGET_TO_PROXY = /* @__PURE__ */ new WeakMap();
var PROXY_TO_TARGET = /* @__PURE__ */ new WeakMap();
var PROXY_TO_OBSERVERS = /* @__PURE__ */ new WeakMap();
var OBSERVER_TO_PROXIES = /* @__PURE__ */ new WeakMap();
var TASK_QUEUE = [];
var SCHEDULER_ID = null;
var isReactiveTarget = (target) => TARGET_TO_PROXY.has(target);
var isReactive = (trk) => PROXY_TO_TARGET.has(trk);
var getReactive = (target) => TARGET_TO_PROXY.get(target);
var getTarget = (tracker) => PROXY_TO_TARGET.get(tracker);
var toRaw2 = (targetOrProxy) => isReactive(targetOrProxy) ? getTarget(targetOrProxy) : targetOrProxy;
var addReactive = (target, proxy) => {
...
var dependTracker = (trk, key) => {
  const current = getCurrentObserver();
  if (!current)
    return;
  if (!OBSERVER_TO_PROXIES.has(current)) {
    OBSERVER_TO_PROXIES.set(current, /* @__PURE__ */ new Map());
  }
  if (!PROXY_TO_OBSERVERS.has(trk)) {
    PROXY_TO_OBSERVERS.set(trk, /* @__PURE__ */ new Map());
  }
  const observers = PROXY_TO_OBSERVERS.get(trk);
  const trackers = OBSERVER_TO_PROXIES.get(current);
  if (!observers.has(key)) {
    observers.set(key, /* @__PURE__ */ new Set());
  }
  if (!trackers.has(key)) {
    trackers.set(key, /* @__PURE__ */ new Set());
  }
...
var dependTarget = (target, key) => {
  const trk = getReactive(target);
  if (trk) {
    dependTracker(trk, key);
  }
};
var trackerChanged = (trk, key) => {
  const observers = PROXY_TO_OBSERVERS.get(trk);
...
var targetChanged = (target, key) => {
  trackerChanged(getReactive(target), key);
};

// src/reactivity/Observer.ts
var GLOBAL_STACK = [];
function getCurrentObserver() {
  return GLOBAL_STACK[GLOBAL_STACK.length - 1];
}
var ObservableItem2 = class {
  static create(...args) {
    return new ObservableItem2(...args);
  }
  constructor(options) {
    ...
    this.alive = true;
    this.dirty = true;
    this.cachedVal = void 0;
    this.hooks = {
      ["onGet" /* onGet */]: options["onGet" /* onGet */],
      ["onSet" /* onSet */]: options["onSet" /* onSet */],
      ["onChange" /* onChange */]: options["onChange" /* onChange */],
      ["onImmediateChange" /* onImmediateChange */]: options["onImmediateChange" /* onImmediateChange */]
    };
  }
  callHook(name) {
  ...
  setDirty() {
    if (this.alive && !this.dirty) {
      ...
      this.callHook("onImmediateChange" /* onImmediateChange */);
      schedule(() => this.notifyChange());
    }
  }
  notifyChange() {
    this.callHook("onChange" /* onChange */);
    this.prev = void 0;
  }
  run() {
    if (!this.alive)
...
function Observer(...args) {
  return ObservableItem2.create(...args);
}
...
// src/reactivity/typeHandlers.ts
var makeOptions = (handlerOptions) => {
  const {
    name,
    target,
    key,
...
var optionGet = (options, target, key, receiver) => {
...
var optionSet = (options, target, key, value) => {
...
var optionDelete = (options, target, key) => {
  targetChanged(target, key);
  targetChanged(target, KEYS_SYMBOL);
  options?.onDelete?.(
    makeOptions({
...
var optionHasOwnKeys = (options, target, key) => {
...
function defaultHandlers(options) {
  ...
  return {
    get: (target, prop, receiver) => {
      ...
      if (needsProxy({ target, key: prop, value }) && options?.deep && Object.getOwnPropertyDescriptor(target, prop)?.configurable) {
...
var objectHandler = (options) => ({
  ...defaultHandlers(options)
});
var arrayHandler = (options) => ({
  ...defaultHandlers(options)
});
var typeHandlers_default = Object.freeze({
...
function Reactive(target, options) {
  if (isReactiveTarget(target))
    return getReactive(target);
  if (isReactive(target))
    return target;
  const opts = options || {};
  const newOptions = { deep: true, ...opts };
  const type = Reactive.getStringType(target);
  if (!Reactive.hasHandler(type))
    return target;
...
Reactive.hasHandler = (type) => !!typeHandlers_default[type];
Reactive.typeHandler = (type, hooks) => typeHandlers_default[type]?.(hooks);

// src/reactivity/Watcher.ts
var import_cloneDeep = __toESM(require_cloneDeep());
function Watcher(get, handler, options) {
  let getter = get;
  if (options?.deep) {
    getter = () => (0, import_cloneDeep.default)(get());
  }
  const obs = Observer({
    get: getter,
...
// src/integrations/MadroneState.ts
function describeComputed(name, config, options) {
  let getter;
  let setter;
  if (config.cache) {
    const cp = Computed({
...
function describeProperty(name, config, options) {
...
// src/integrations/MadroneVue2.ts
var FORBIDDEN = /* @__PURE__ */ new Set(["__proto__", "__ob__"]);
var VALUE = "value";
var MadroneVue2_default = (opts) => {
  const { observable, set } = opts;
  const reactiveMappings = /* @__PURE__ */ new WeakMap();
  ...
  const reactiveSet = set ? (item) => set(item, VALUE, item[VALUE] + 1) : (item) => {
    item[VALUE] += 1;
  };
  const depend = (cp, key) => {
  ...
  function describeComputed2(name, config) {
    return describeComputed(name, config, options);
  }
  function describeProperty2(name, config) {
    return describeProperty(name, config, options);
  }
  function defineComputed2(target, name, config) {
    return defineComputed(target, name, config, options);
  }
  function defineProperty2(target, name, config) {
    return defineProperty(target, name, config, options);
  }
  return {
    watch: Watcher,
    describeProperty: describeProperty2,
    defineProperty: defineProperty2,
    describeComputed: describeComputed2,
    defineComputed: defineComputed2
  };
};

// src/integrations/MadroneVue3.ts
var MadroneVue3_default = ({ reactive: reactive2, computed: computed2, watch: watch2, toRaw: toRaw3 } = {}) => {
  function describeComputed2(name, config) {
    let getter;
    let setter;
    if (config.cache) {
      const cp = computed2(config);
      getter = function get() {
        objectAccessed(this);
        return cp.value;
      };
  ...
  function defineComputed2(target, name, config) {
    Object.defineProperty(target, name, describeComputed2(name, config));
  }
  function describeProperty2(name, config) {
    const tg = { value: config.value };
    const atom = reactive2(tg);
    return {
      configurable: config.configurable,
      enumerable: config.enumerable,
      get: function get() {
  ...
  function defineProperty2(target, name, config) {
    Object.defineProperty(target, name, describeProperty2(name, config));
  }
  return {
    toRaw: toRaw3,
    watch: watch2,
...
function applyClassMixins(base, mixins) {
...
// src/decorate.ts
var itemMap = /* @__PURE__ */ new WeakMap();
...
function trackTargetIfNeeded(target) {
  if (!itemMap.has(target)) {
    itemMap.set(target, /* @__PURE__ */ new Set());
  }
}
...
function computedIfNeeded(target, key, descriptor, options) {
  const pl = getIntegration();
...
function decorateComputed(target, key, descriptor, options) {
...
computed.configure = function configureComputed(descriptorOverrides) {
  return (target, key, descriptor) => decorateComputed(target, key, descriptor, { descriptors: descriptorOverrides });
};
function reactiveIfNeeded(target, key, options) {
  const pl = getIntegration();
  if (pl && !checkTargetObserved(target, key)) {
    setTargetObserved(target, key);
    define(target, key, {
...
function decorateReactive(target, key, options) {
...
reactive.configure = function configureReactive(descriptorOverrides) {
  return (target, key) => decorateReactive(target, key, { descriptors: descriptorOverrides });
};

// src/index.ts
addIntegration(MadroneState_default);
var Madrone = {
  use: addIntegration,
...
var src_default = Madrone;
export {
  MadroneState_default as MadroneState,
  MadroneVue2_default as MadroneVue2,
  MadroneVue3_default as MadroneVue3,
  applyClassMixins,
  auto,
  classMixin,
  computed,
